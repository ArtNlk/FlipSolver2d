...
namespace
{
const double TempScale = 0.436906689453125;
}
                         ...
                         double tol)
{
    ...
    for (int i = 0; i < iterLimit; i++) {
        ...
        if (err <= tol) {
            ...
            std::ofstream file("iters.txt", std::ios::app);
...
void LinearSolver::dampedJacobiThread(LinearSolver* solver, const Range range, const MaterialGrid &materials, std::vector<double> &vout, const std::vector<double> &pressures, const std::vector<double> &rhs)
{
    ...
    for(int i = range.start; i < range.end; i++)
    {
        ...
        for(int idx : neighbors)
        {
            ...
            result -= TempScale*pressures[idx]*materials.isFluid(static_cast<ssize_t>(idx));
            diag += TempScale*!materials.isSolid(static_cast<ssize_t>(idx));
        }
    ...
    for(int idx = range.start; idx < range.end; idx++)
    {
        ...
        if(materials.isFluid(static_cast<ssize_t>(idx)))
        {
            ...
            for(int neighborIdx : neighbors)
            {
                ...
                result -= TempScale*vmul[neighborIdx]*materials.isFluid(static_cast<ssize_t>(neighborIdx));
                diag += TempScale*!materials.isSolid(static_cast<ssize_t>(neighborIdx));
            }
                                ...
                                std::vector<double> &smoothedPressureData, int iterCount)
{
    ...
    {
        for(size_t idx = 0; idx < maskGrid.linearSize(); idx++)
        {
            if(!maskGrid.data().at(idx) || !currentMaterialGrid.isFluid(static_cast<ssize_t>(idx)))
            {
                smoothedPressureData[idx] += (rhsData[idx]/TempScale);
                continue;
            }
            //const auto weights = solver->getMultigridMatrixEntriesForCell(materials,i);
            ...
            for(int neighborIdx : neighbors)
            {
                ...
                result -= TempScale*smoothedPressureData[neighborIdx]*currentMaterialGrid.isFluid(static_cast<ssize_t>(neighborIdx));
                diag += TempScale*!currentMaterialGrid.isSolid(static_cast<ssize_t>(neighborIdx));
            }
